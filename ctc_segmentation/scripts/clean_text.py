...
def word_to_num(word):
    ...
    while match is not None:
        num_word = num2words.num2words(float(match.group().replace(',', '.')), lang='en')#.lower()
    ...
    while match is not None:
        num_word = num2words.num2words(int(match.group()), lang='en')
        before = result[:match.start()]
        after = result[match.end():]
        result = ' '.join([before, num_word, after])
        match = int_pattern.search(result)
...
def clean_sentence(sentence):
    ...
    sentence = re.sub(r'([0-9]+)(\.)([0-9]+)', r' \1 point \3', sentence)
